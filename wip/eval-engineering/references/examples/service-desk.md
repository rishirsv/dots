# Service-Desk Evaluation Knowledge Example
...
Create `skills/.service-desk/AGENTS.md`. Add only these proven
...
validator only because two distinct Tasks now use them. Update `AGENTS.md` to
route agents to those items.

## Resulting companion guidance
...
Its AGENTS.md states when the knowledge applies and gives exact project paths,
commands, state rules, and known limits. It can contain project-specific Task,
Environment, data, Verifier, task-running, and calibration guidance. It does not say
only “follow eval-engineering,” and it does not copy broad guidance without a
project-specific adaptation.

The guidance does not include either Task's request, selected records, expected
...
Task 3 tests whether the companion AGENTS.md works. A new author should be able to find
