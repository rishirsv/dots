...
Task needs. Put Task-specific choices in the collocated `Task.md`. Follow
[Companion guidance](companion-guidance.md) when retaining proven project methods.
...
1. Check trial isolation in the way this Environment needs. For reused mutable
   ...
   reset check is needed. For an approved live sandbox, verify its dedicated
   identity, safe starting state, allowed effects, and cleanup. Create another
   fresh trial only when nondeterminism, shared state, or flaky setup is a risk.
...
After the approved run, use [Calibration](calibration.md) to check whether the
Environment created the intended decision.

For schema fidelity, use [tool-schema comparison](../scripts/compare_tool_schemas.py)
on supplied fragments; it does not resolve external `$ref` targets. For SQLite
state evidence, use the [read-only snapshot helper](../scripts/snapshot_sqlite_state.py).
