...
import { collectTurnEvents, summarizeTrajectory, type Trajectory } from "./trajectory.ts";

describe("collectTurnEvents", () => {
  it("collects final text, completion, and token usage for the selected turn", () => {
    const turn = collectTurnEvents(
      [
  ...
  it("records unavailable token evidence when the token event is missing", () => {
    const turn = collectTurnEvents([event("turn/completed", { threadId: "thread-1", turn: { id: "turn-1" } })], { threadId: "thread-1", turnId: "turn-1" });
  ...
  it("preserves unknown observed App Server methods", () => {
    const turn = collectTurnEvents([event("item/newFamily/delta", { threadId: "thread-1", turnId: "turn-1", value: true })], { threadId: "thread-1", turnId: "turn-1" });
  ...
  it("summarizes a trajectory without writing another fact file", () => {
    ...
    assert.deepEqual(summarizeTrajectory(trajectory), {
      turn_count: 1,
      item_count: 1,
