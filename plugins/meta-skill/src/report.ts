...
interface ReportAppModelV1 {
  ...
  cases: ReportAppCaseV1[];
}

interface ReportAppCaseV1 {
  id: string;
  folder: string;
  title: string;
  subtitle: string;
...
function toReportAppCase(report: RunReport, item: RunReportCase): ReportAppCaseV1 {
...
function caseSubtitle(item: RunReportCase): string {
  return [item.type, item.topics.join(", ")].filter(Boolean).join(" / ") || item.folder;
}

function attemptsForCase(item: RunReportCase): ReportAppCaseV1["attempts"] {
...
function criteriaRowsForCase(item: RunReportCase): Array<{ label: string; value: string }> {
...
function evidenceRowsForCase(report: RunReport, item: RunReportCase): ReportAppCaseV1["evidence"] {
  const rows: ReportAppCaseV1["evidence"] = [];
  ...
  for (const row of [...report.tests, ...report.judges, ...report.feedback]) {
    if (!eventMatchesCase(row, item)) continue;
    rows.push({
...
function eventMatchesCase(row: EventEnvelope, item: RunReportCase): boolean {
  const payload = row.payload || {};
  const candidates = [row.case_id, payload.case_id, payload.case_folder, payload.folder, payload.id].filter((value) => value !== undefined && value !== null).map(String);
  return candidates.includes(item.id) || candidates.includes(item.folder);
}

function reviewReasonsForCase(item: RunReportCase, evidence: ReportAppCaseV1["evidence"]): string[] {
  const reasons = new Set<string>();
  if (item.status === "failed") reasons.add("Failed case evidence needs attention first.");
  if (item.no_verdict_recorded) reasons.add("Execution completed; no deterministic test, judge, or human feedback verdict is recorded for this item.");
...
function defaultSelectedCase(cases: ReportAppCaseV1[]): ReportAppCaseV1 | undefined {
...
function renderCaseRail(cases: ReportAppCaseV1[], selectedId?: string): string {
  if (!cases.length) return '<p class="empty">No cases match this filter.</p>';
  return cases
...
function renderCaseDetail(item: ReportAppCaseV1, model: ReportAppModelV1): string {
...
function renderAttemptPreviews(item: ReportAppCaseV1, model: ReportAppModelV1): string {
  const fallback: ReportAppCaseV1["attempts"][number] = { label: model.run.run_source.label, execution_status: "unknown", status: "unknown", final_preview: "No final output recorded.", source_kind: model.run.run_source.kind, raw_links: [] };
...
function renderMetadataRail(item: ReportAppCaseV1 | undefined, model: ReportAppModelV1): string {
...
function renderCaseTokenUsage(item: ReportAppCaseV1): string {
  if (!item.attempts.length) return '<p class="empty">No token usage recorded.</p>';
...
function renderRawLinks(item: ReportAppCaseV1): string {
...
function renderCaseCard(item: RunReportCase): string {
  const criteria = item.criteria;
