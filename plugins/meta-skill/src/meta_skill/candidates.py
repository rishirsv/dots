...
def resolve_candidate(project, worktree_root, run_id_value, manifest, candidate, *, skill_id_value=None):
    ...
    return {
        ...
        "variant": candidate.get("variant") or candidate_id,
...
def snapshot_candidate(run_dir, candidate_info):
    ...
    snapshot_root = run_dir / "inputs" / "variants" / candidate_id
    ...
    if prompt:
        raw_path = prompt.get("path") if isinstance(prompt, dict) else prompt
        prompt_path = Path(raw_path).expanduser()
        if not prompt_path.is_absolute():
            prompt_path = Path(candidate_info["cwd"]) / prompt_path
        prompt_path = prompt_path.resolve()
        if not prompt_path.is_file():
            raise CliError(f"variant {candidate_id} prompt not found: {prompt_path}", 2)
        prompt_text = prompt_path.read_text()
        frozen_prompt = snapshot_root / "prompt" / "prompt.md"
        frozen_prompt.parent.mkdir(parents=True, exist_ok=True)
        frozen_prompt.write_text(prompt_text)
        prompt_snapshot = {"path": "prompt/prompt.md", "digest": payload_digest(frozen_prompt)}
    snapshot = {
        "variant": candidate_info.get("variant") or candidate_id,
        "candidate": candidate_id,
        ...
        "model": candidate_info.get("model"),
        "prompt": prompt_snapshot,
        "tools": list(candidate_info.get("tools") or []),
        "plugins": list(candidate_info.get("plugins") or []),
        "retrieval": candidate_info.get("retrieval"),
    }
