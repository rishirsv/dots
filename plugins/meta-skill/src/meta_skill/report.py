...
def build_report(raw_run, *, blind_pending_human=False):
    ...
    checklist_default = (run.get("format") or suite.get("format")) == "scenario"
    checklist_scores = defaultdict(list)
    if checklist_default:
        for trial in trials:
            for grade in trial.get("grades") or []:
                if (grade.get("grader") or {}).get("kind") == "weighted_checklist" and isinstance(grade.get("score"), (int, float)):
                    checklist_scores[trial.get("candidate")].append(float(grade["score"]))
    ...
    report = {
        "schema_version": 2,
        "format": "scenario" if checklist_default else "custom",
        ...
        "baseline_variant": run.get("baseline_variant") or baseline_candidate,
        ...
        "variants": run.get("variants") or run.get("candidates") or [],
        "variant_scores": {
            key: {
            ...
            for key, values in sorted(checklist_scores.items())
        },
...
def render_markdown(report):
    ...
    if comparisons:
        ...
        lines.append("No variant comparison is available.")
    ...
    lines += [
        ...
        *(
            ["- Evaluation verdict: not applicable; default grading reports weighted scores"]
            if report.get("format") == "scenario"
            else [f"- Variant evaluation passed: {'yes' if report.get('evaluation_passed') else 'no'}"]
        ),
        ...
        *(
        ...
        f"- Baseline variant: {report.get('baseline_variant') or report.get('baseline_candidate') or 'none'}",
        f"- Variants: {', '.join(row.get('variant') or row.get('candidate') or '' for row in report.get('variants') or []) or 'none'}",
