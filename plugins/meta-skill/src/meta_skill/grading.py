...
from .scenario_format import weighted_checklist_score
...
def normalize_graders(case):
    ...
    for index, grader in enumerate(explicit, 1):
        if not isinstance(grader, dict) or grader.get("kind") not in {"code", "model", "human", "weighted_checklist"}:
...
def _weighted_checklist_grade(run_dir, run, state, root, case, grader, generation_id, model, reasoning_effort):
    trial = trial_path(run_dir, state["trial_id"])
    response = trial / "response.md"
    events = trial / "events.jsonl"
    artifacts_dir = trial / "artifacts"
    artifact_paths = sorted(path for path in artifacts_dir.rglob("*") if path.is_file()) if artifacts_dir.exists() else []
    criteria = case.get("criteria") or {}
    checklist = criteria.get("checklist") or []
    guidance = (
        "Use default weighted-checklist grading exactly. Score every checklist item independently from 0 "
        "through its max_score, allowing partial credit. Put the awarded points in checks[].level and use "
        "the criterion name exactly. Do not add criteria or hard gates. The top-level label is only a summary.\n\n"
    ...
    detail = judge_output(
        ...
        expectations=[item["description"] for item in checklist],
        events_text=None,
        cwd=run_dir,
    ...
    awards = [
        {"name": row.get("name"), "score": row.get("level"), "evidence": row.get("evidence")}
        for row in detail.get("checks") or []
    ]
    weighted = weighted_checklist_score(criteria, awards)
    score = weighted["score"]
    label = "pass" if score == 1 else "fail" if score == 0 else "partial"
    return _grade_row(
        run_dir,
        run,
        state,
        generation_id=generation_id,
        grader=grader_descriptor(grader, kind="weighted_checklist", default_id="weighted-checklist"),
        grade_status=label,
        score=score,
        checks=weighted["checks"],
        rationale=detail.get("rationale") or "",
        evidence_refs=[response, *artifact_paths, trial / f"judge-{generation_id}.jsonl"],
        detail={
            "grading_model": "weighted-checklist",
            "earned": weighted["earned"],
            "available": weighted["available"],
...
def _grade_trial(run_dir, run, state, case, generation_id, model, reasoning_effort, annotations):
    ...
    root = run_dir / "inputs" / "scenarios" / state["eval_id"]
    ...
    for grader in normalize_graders(case):
        ...
        elif grader["kind"] == "weighted_checklist":
            rows.append(_weighted_checklist_grade(run_dir, run, state, root, case, grader, generation_id, model, reasoning_effort))
