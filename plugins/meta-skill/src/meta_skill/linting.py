...
from .manifest import case_dir, load_manifest, suite_path
...
def lint_suite(raw_suite):
    ...
    for case in cases:
        ...
        if isinstance(prompt, dict) and prompt.get("path"):
            task_file = case_dir(suite, case_id) / "task.md"
    ...
    return {
        "ok": True,
        "suite": str(suite),
        "shape": "scenarios" if manifest.get("format") == "scenario" else "custom-evals-v2",
        "stats": {
            "tasks": stats["tasks"],
