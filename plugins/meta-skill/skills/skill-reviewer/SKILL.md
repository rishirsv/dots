...
when the answer requires new task runs, execution-variant comparisons, grading, or run
...
payload or package hygiene. It also runs sentence pruning across the effective
runtime and checks whether invocation mode, leading words, completion criteria,
and skill splits produce a predictable process. Within those areas, select only
checks that apply to the skill. Load the relevant domain module when the skill
