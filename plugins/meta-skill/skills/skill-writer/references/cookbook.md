...
| New non-trivial skill, trigger-risky skill, or source material with realistic prompts and expected behavior | Eval manifest handoff | `.<skill-name>/evals.json` suite manifest outside runtime payload | Confirm prompts are realistic, objective enough, and routed to `skill-evaluator` for runs |
