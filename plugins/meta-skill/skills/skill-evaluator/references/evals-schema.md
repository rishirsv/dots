# Custom Evaluation Suite Schema

Read this only after [custom-graders.md](custom-graders.md) selects an independent
grader. Ordinary evaluations use standard scenario directories and
[weighted-checklist.md](weighted-checklist.md).

Create custom suites at
`<skill-name>/.<skill-name>/custom-evals/evals.json`.
  ...
  "variants": [
    {
      ...
      "display": "Without skill",
      "skill": {"kind": "none"}
    },
    {
      "variant": "current",
      "display": "Current skill",
      "skill": {"kind": "current_worktree", "ref": "."}
    }
  ],
  "evals": [
    {
...
When `variants` is omitted, MetaSkill supplies the no-skill and current-skill
variants shown above. Expectations without an explicit grader are advisory
...
Each variant needs a unique `variant`, optional `display`, and `skill`:

| `skill.kind` | Required skill field |
|---|---|
| `none` | No `ref` or `path`; reserved for variant `without-skill`. |
| `current_worktree` | Optional `ref: "."`; freezes the current skill tree. |
| `local_path` | `path` or `ref` naming the local variant directory. |
| `git_ref` | `ref` naming a commit, tag, branch, or other Git revision. |

For a custom revision comparison, declare both versions and run with `--baseline
current --variants proposed`. Variant IDs select declared sources; they do
...
declared fixtures, and the frozen variant skill. Expected output, criteria,
...
```bash
metaskill eval run \
  --suite <skill-name>/.<skill-name>/custom-evals/evals.json \
  --check --json
```
