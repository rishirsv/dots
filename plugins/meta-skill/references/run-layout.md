...
Authored standard scenarios live in visible `evals/`, which packaging
excludes. Experiment settings, custom evals, and generated state live in the
ignored `.<skill-name>/` companion.
...
<skill-name>/
  ...
  .<skill-name>/
    ...
    worktrees/<run-id>/
      variants/<variant-id>/    # temporary git worktrees
    tmp/<run-id>/
...
cases, variants, and trials changes:

- request: `inputs/scenarios/<scenario-id>/task.md`
...
- provenance: `run.json`, `inputs/suite.json`, and variant snapshots
...
benchmark split, baseline, variants, case digests, repetitions, planned
...
Variant payloads are stored once per run, not copied into each trial.
...
fixtures, a workspace-local result location, and a staged variant payload
...
They are not a security sandbox and cannot run variant comparisons; native
packets are limited to one-variant observations. The unattended Codex Exec
comparison lane adds a workspace-write sandbox, ignores user configuration and
rules, disables plugins, apps, and memories, records an empty plugin inventory, supplies only the frozen variant
...
Only `git_ref` variant sources use detached Git worktrees during
materialization. Current-worktree and local-path variants are frozen by
...
completed trial only when its model, case digest, variant payload digest, and
