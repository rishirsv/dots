...
function countTokenUsage(scenarios) {
    ...
    let available = 0;
    let unavailable = 0;
    for (const sideName of ["candidate", "release"]) {
        const summaries = scenarios.flatMap((scenario) => scenario.sides.filter((side) => side.side === sideName).map((side) => side.token_usage).filter((usage) => Boolean(usage)));
        if (!summaries.length)
            continue;
...
async function readSideTokenUsage(runRoot, evidencePath, legacy) {
...
function toReportAppModel(report) {
    ...
    const variants = ["candidate", "release"]
        .filter((side) => report.scenarios.some((scenario) => scenario.sides.some((attempt) => attempt.side === side)))
        .map((side) => ({ id: side, label: side === "candidate" ? "Candidate" : "Release", kind: side === "candidate" ? "working_tree" : "release" }));
    return {
        schema_version: 1,
...
function comparisonForScenario(report, scenario) {
...
function rawLinksForAttempt(side) {
    if (!side.evidence_path)
        return [];
    return ["final.md", "turns.jsonl", "usage.json", "rpc.jsonl"].map((file) => ({ label: file, href: `${side.evidence_path}/${file}`.split(node_path_1.default.sep).join("/") }));
}
...
function evidenceRowsForScenario(report, scenario) {
    const rows = [];
    for (const side of scenario.sides) {
        if (side.failure_classification || side.error)
            rows.push({ type: side.side, status: side.status, detail: side.failure_classification || side.error || "" });
    }
...
function reviewReasonsForScenario(scenario, evidence) {
    ...
    for (const side of scenario.sides) {
        if (side.error)
            reasons.add(side.error);
        if (side.failure_classification)
            reasons.add(sentenceStatus(side.failure_classification));
    }
    return [...reasons];
}
...
function renderAttemptPreviews(scenario, model) {
    ...
    const sides = model.variants.some((variant) => variant.id === "release") ? ["candidate", "release"] : ["candidate"];
    return sides
        .map((side) => {
        const attempt = attemptsBySide.get(side);
        if (!attempt && side === "release")
            return '<div class="answer"><h3>Release</h3><p class="empty">No release attempt in this run.</p></div>';
        if (!attempt)
            return '<div class="answer"><h3>Candidate</h3><p class="empty">No candidate attempt in this run.</p></div>';
...
function comparisonLabel(value) {
    if (value === "not_comparable")
        return "No release comparison";
    if (value === "candidate_regresses")
        return "Release better";
    if (value === "candidate_beats_release")
        return "Candidate better";
    if (value === "both_fail")
        return "Both failed";
    if (value === "no_regression")
