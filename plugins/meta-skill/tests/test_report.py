...
class ReportTests(unittest.TestCase):
    def test_delta_failed_evidence_and_markdown_share_one_model(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report["delta_totals"], {"observed_improvement": 1})
            self.assertEqual(report["comparisons"][0]["delta"], "observed_improvement")
            ...
            self.assertGreater(summaries["current"]["pass_rate_interval"][0], 0)
            self.assertEqual(summaries["current"]["pass_rate_interval"][1], 1)
            ...
            self.assertIn("1 observed improvements", markdown)
            self.assertIn("Diagnostic observation", markdown)
    ...
    def test_any_trial_policy_controls_candidate_evaluation(self):
        with tempfile.TemporaryDirectory() as tmp:
            run = fixture(Path(tmp))
            model_path = run / "run.json"
            model = json.loads(model_path.read_text())
            model["repetition_policy"] = "any_trial"
            for candidate in ("no-skill", "current"):
                trial_id = f"a.{candidate}.t2"
                model["trials"].append({
                    "trial_id": trial_id,
                    "eval_id": "a",
                    "candidate": candidate,
                ...
                (trial / "response.md").write_text(candidate)
                (trial / "events.jsonl").write_text("")
                jsonl(trial / "grades.jsonl", [{
                    "trial_id": trial_id,
                    "metric": "quality",
                    "grader": {"kind": "model", "id": "judge"},
                    "grade_status": "partial" if candidate == "current" else "fail",
                    "rationale": "failed repetition",
                }])
    ...
    def test_run_is_not_terminal_while_declared_human_review_is_pending(self):
        with tempfile.TemporaryDirectory() as tmp:
            run = fixture(Path(tmp))
            suite_path = run / "inputs" / "suite.json"
            suite = json.loads(suite_path.read_text())
            suite["evals"][0]["graders"] = [
                {"kind": "human", "id": "artifact-review", "metric": "artifact-correctness"}
            ]
            write(suite_path, suite)
            for candidate in ("no-skill", "current"):
                ...
                with path.open("a") as handle:
                    handle.write(json.dumps({
                        "trial_id": f"a.{candidate}.t1",
                        "metric": "artifact-correctness",
                        "grader": {"kind": "human", "id": "artifact-review"},
            ...
            for candidate, label in (("no-skill", "fail"), ("current", "pass")):
                ...
                with path.open("a") as handle:
                    handle.write(json.dumps({
                        ...
                        "grader": {"kind": "human", "id": "artifact-review"},
                        "grade_status": label,
                        "rationale": "Blind review complete.",
                    }) + "\n")
            complete = build_report(str(run))
            self.assertTrue(complete["grading_complete"])
            self.assertTrue(complete["terminal"])
    ...
    def test_explicit_non_none_baseline_drives_candidate_comparison(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report["comparisons"][0]["delta"], "observed_regression")
    ...
    def test_list_runs_uses_live_artifacts_and_rejects_old_layout(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(listed[0]["delta_totals"], {"observed_improvement": 1})
    ...
    def test_readiness_report_requires_broad_isolated_design_and_uses_paired_inference(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cases = [
                {"id": f"case-{index}", "coverage": ["core" if index < 10 else "boundary"], "graders": [{"kind": "human", "id": "review", "metric": "correctness"}]}
            ...
            for case in cases:
                for repetition in range(1, 2):
                    for candidate, label in (("no-skill", "fail"), ("current", "pass")):
                        ...
                        trial = run / "trials" / trial_id
                        write(trial / "state.json", {"trial_id": trial_id, "eval_id": case["id"], "candidate": candidate, "repetition": repetition, "status": "completed"})
                        (trial / "response.md").write_text(candidate)
                        (trial / "events.jsonl").write_text("")
                        jsonl(trial / "grades.jsonl", [{"trial_id": trial_id, "metric": "correctness", "grader": {"kind": "human", "id": "review"}, "grade_status": label, "rationale": label}])
            write(run / "run.json", {
                "schema_version": 2,
                "run_id": "run",
                ...
                "task_executor": {
                    ...
                    "isolation": {"supports_baseline_effect": True},
                },
                "judge_executor": None,
                "baseline_candidate": "no-skill",
                "candidates": candidates,
            ...
            write(run / "inputs" / "suite.json", {"schema_version": 2, "evaluation_mode": "readiness", "validity_review": {"status": "pass", "notes": "Reviewed."}, "coverage_requirements": ["core", "boundary"], "evals": cases})
            report = build_report(str(run))
            self.assertTrue(report["claim_supported"])
            self.assertEqual(report["paired_inference"][0]["conclusion"], "supported_improvement")
            self.assertLess(report["paired_inference"][0]["exact_mcnemar_p"], 0.05)
            self.assertEqual(report["delta_totals"], {"case_improvement": 20})
            markdown = render_markdown(report)
            self.assertIn("Comparative estimate with paired inference", markdown)

            model = json.loads((run / "run.json").read_text())
            model["validity_review"] = {
                "status": "unknown",
                "notes": "Harness fidelity has not been confirmed.",
            }
            write(run / "run.json", model)
            invalid = build_report(str(run))
            self.assertFalse(invalid["claim_supported"])
            self.assertIn("validity review is not passed", " ".join(invalid["coverage_limits"]))
            self.assertIn("supported_improvement", markdown)
