...
class RunnerTests(unittest.TestCase):
    ...
    def test_codex_exec_rejects_unsupported_variant_provider(self):
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "skill"
            skill(target)
            manifest = target / ".demo" / "custom-evals" / "evals.json"
            ...
            manifest.write_text(json.dumps({
                ...
                "variants": [{
                    "variant": "claude",
                    "model": {"provider": "unsupported", "name": "external-model"},
                    "skill": {"kind": "current_worktree"},
                }],
                "evals": [{"id": "case-1", "type": "capability", "prompt": "Do it"}],
            }))
            ...
            self.assertIn("unsupported model providers", caught.exception.message)
