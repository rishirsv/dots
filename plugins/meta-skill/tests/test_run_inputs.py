...
class RunInputTests(unittest.TestCase):
    def test_inline_and_file_inputs_freeze_hidden_material_but_stage_only_visible_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            frozen_case = {**frozen["evals"][0], "case_root": str(run_dir / "inputs" / "scenarios" / "a")}
            ...
            self.assertTrue((run_dir / "inputs" / "scenarios" / "a" / "judge.md").is_file())
            self.assertTrue((run_dir / "inputs" / "scenarios" / "a" / "expected.md").is_file())
