"""Deterministic tests for the advisor skill's package builder.

Kept under plugins/meta-skill/tests/ (not packaged into the advisor skill) so the
portable advisor skill ships without test code. Run with:

    python3 plugins/meta-skill/tests/test_advisor_package.py
"""
...
SCRIPT = REPO_ROOT / "plugins" / "dots" / "skills" / "advisor" / "scripts" / "advisor_package.py"
...
class AssistPackageTests(unittest.TestCase):
    def run_package(self, *args: str, cwd: Path, check: bool = True) -> subprocess.CompletedProcess[str]:
        ...
        if check and result.returncode != 0:
            self.fail(f"advisor_package.py failed\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}")
        return result
    ...
    def test_task_file_complete_prompt_becomes_base_prompt(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.run_package(
                ...
                str(root),
                "--name",
                "advisor-authored-task",
                cwd=root,
            )

            prompt = (root / "advisor-authored-task" / "prompt.md").read_text(encoding="utf-8")
    ...
    def test_prompt_file_preserves_authored_prompt_without_generated_wrapper(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            prompt = (root / "advisor-authored" / "prompt.md").read_text(encoding="utf-8")
    ...
    def test_generated_prompt_uses_task_shaped_default_labels(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            prompt = (root / "advisor-generated" / "prompt.md").read_text(encoding="utf-8")
    ...
    def test_context_map_suppresses_mechanical_file_reasons(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            prompt = (root / "advisor-context-map" / "prompt.md").read_text(encoding="utf-8")
    ...
    def test_single_heading_task_file_still_gets_context_wrapper(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.run_package(
                ...
                str(root),
                "--name",
                "advisor-single-heading",
                cwd=root,
            )

            prompt = (root / "advisor-single-heading" / "prompt.md").read_text(encoding="utf-8")
            self.assertIn("Provide a focused second opinion.", prompt)
    ...
    def test_dry_run_previews_without_writing_package(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((root / "advisor-dry-run").exists())
    ...
    def test_token_budget_blocks_oversized_package(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((root / "advisor-oversized").exists())
    ...
    def test_allow_oversized_writes_over_budget_package(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((root / "advisor-allow-oversized" / "prompt.md").exists())
