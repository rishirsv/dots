...
class CandidateTests(unittest.TestCase):
    def test_local_path_candidate_snapshot_is_run_local(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(snapshotted["payload_path"], str(run_dir / "inputs" / "variants" / "local" / "payload"))
    ...
    def test_plugin_snapshot_preserves_shared_reference_layout(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload, run_dir / "inputs" / "variants" / "current" / "payload" / "skills" / "demo")
