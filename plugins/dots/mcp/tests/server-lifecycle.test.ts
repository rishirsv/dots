...
import { defaultBrokerEndpoint, defaultConfig as createDefaultConfig, providerConfig } from "../src/config";
...
test("a drained runtime rejects new model-catalog work before shutdown", async () => {
  ...
  try {
    ...
    expect(await models.json()).toMatchObject({
      error: {
        type: "server_error",
        message: "Portal is draining for a requested service operation",
      },
    });
  ...
  const server = startServer(defaultConfig(), {
    fetchUpstream: async () => {
      ...
      return Response.json({ models: [{
        ...
        supported_in_api: true, supported_reasoning_levels: [], context_window: 200_000,
        tool_mode: "code_mode_only",
      }] });
    },
  });
