...
test("native fetch reaches a proxy-only target, refreshes routing, and never retries a failed route", async () => {
  ...
  try {
    ...
    expect(calls[0]).toMatchObject({ authorization: "Bearer codex-test-only", body: "native request" });
    expect(new URL(calls[0]!.url)).toMatchObject({ hostname: "native-proxy-regression.invalid", pathname: "/responses" });
    route = "DIRECT";
