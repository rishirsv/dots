...
test("retained compaction provides one exact same-agent control binding", () => {
  ...
  expect(prompt).toContain("do not use it with portal_tools or any outer Codex tool");
...
test.skip("requires Bun 1.4 mock timers: fresh multipart compaction gives each acknowledged phase its own handoff budget", async () => {
...
test.skip("requires Bun 1.4 mock timers: a timed-out fresh compaction retains its owner until helper cleanup completes", async () => {
