...
Portal source is private/unpublished (`UNLICENSED` in package.json). No license or trademark availability is asserted for public distribution under the name Portal. No Commander source or assets were copied; its public archive was used read-only as a reference. UPSTREAM.md records the archive digest and absent execution-server checkout.

The project imports the official MCP TypeScript SDK, QuickJS/WASM, pg, ws, jose, Zod, TypeScript and Node type packages at the exact top-level versions declared in package.json. Their complete resolved dependency graph is recorded in `package-lock.json`; dependencies are not committed to the repository, but the per-user installer copies the locally installed tree into its content-addressed runtime. Installation and release review must preserve each dependency's LICENSE/NOTICE and generate a complete SBOM from the reviewed lock; do not treat this file as a completed license audit.
