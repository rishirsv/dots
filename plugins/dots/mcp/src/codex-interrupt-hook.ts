...
export const MANAGED_INTERRUPT_HOOK_START =
  "# Managed by Portal: release the exact Responses request when its Codex turn is interrupted.";
export const MANAGED_INTERRUPT_HOOK_END =
  "# End Portal interrupt lifecycle hook.";
...
export function installCodexInterruptHookCommand(
  ...
  if (managedMarkerCount(text) !== 0 || text.includes(MANAGED_INTERRUPT_HOOK_END)) {
    throw new Error("Codex config already contains a Portal interrupt hook marker");
  }
