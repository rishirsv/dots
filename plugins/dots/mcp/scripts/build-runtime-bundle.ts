...
function embeddedBunExecutable(): string {
  const configured = process.env.PORTAL_EMBEDDED_BUN;
  if (!configured) return realpathSync(process.execPath);
  if (!isAbsolute(configured)) throw new Error("PORTAL_EMBEDDED_BUN must be an absolute path");
...
const launcherName = process.platform === "win32" ? "portal.cmd" : "portal";
const launcher = process.platform === "win32" ? `@echo off
setlocal
chcp 65001 >nul
set "ROOT=%~dp0.."
set "PORTAL_LAUNCHER=%~f0"
...
export PORTAL_LAUNCHER="$invoked"
