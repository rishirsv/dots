---
name: why
description: "Use for 'why does X work this way', design rationale, regressions, postmortems, or data-backed thresholds. Searches available evidence across source control, tickets, docs, chat, observability, errors, and analytics, then returns a cited read on decisions and tradeoffs. Use how for runtime behavior."
---
...
When `gh` is installed and authenticated, pull PR bodies and discussion for any substantive commits:
...
Before spawning investigators, inspect the runtime's available tools,
connectors, apps, and MCP resources. Use the available-tools map and component
descriptions the host provides.
...
Source control is always available through git; use `gh` when it is installed
and authenticated. For the other six, classify using the component name,
instructions, tool names, and resource descriptors. If a component could fit
more than one category, choose the one matching its primary evidence. Record
ambiguous cases in the coverage map.
...
Launch matching investigators together so they run concurrently, up to the
runtime's concurrency limit. Start the remaining investigators as slots become
available. One investigator per category lets each specialize in one tool's
query vocabulary and result shape. Don't ask one agent to cover multiple
evidence categories.

Use fresh tool-capable subagents. Give each investigator access to the connector
or source it owns. Keep the work read-only: investigators may query external
systems but must not write files, change repository state, or mutate external
data.
...
Spawn one investigator per category that has a matching tool or connector. Each owns exactly one evidence category and its matching source.
...
- **No matching tool or connector is available for that category** in this environment. Flag this as a gap, not a choice. Example: "Real-time team chat skipped. No matching connector available, so the conversational record was not searchable."
...
Spawn one tool-capable synthesizer subagent. Keep the work read-only, but retain
access to the queried sources because the synthesizer's quality check
spot-verifies citations.
...
- **Collapsing investigators into one agent**. Each evidence category has its own query vocabulary, result shape, and pitfalls; pooling them dilutes specialization and makes coverage harder to reason about. Always one investigator per category.
