...
checks = [
    {
        ...
        "passed": bool(image_embeds),
        "evidence": f"found {len(image_embeds)} Markdown image embeds",
    },
    {
        "name": "rejects-text-only-screen-list",
        "passed": "text-only" in lower and ("not enough" in lower or "does not satisfy" in lower),
        "evidence": "looked for explicit text-only list rejection",
    },
    {
        "name": "verifies-live-pr-body",
        "passed": "re-query" in lower or "verify the live pr body" in lower or "gh pr view" in lower,
