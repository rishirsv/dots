...
For web implementation details, read only the needed examples in
[typography-recipes.md](../assets/typography-recipes.md).
...
Use existing audit or QA findings to guide corrections when those workflows are
active. Keep their severity, reporting, and iteration rules rather than starting
a second review loop. Report observations precisely and retain strengths that a
