...
For new work, turn the grounded brief into a compact direction: composition,
type roles, palette, imagery, and interaction. For a focused refinement, keep
the settled direction and change only what the task requires.

Implement the main experience using real content and the relevant craft references.
Default to local HTML for standalone web prototypes. Use a native preview when
the decision depends on platform controls, gestures, or text scaling. Use the
existing stack and design system for product changes. Include the controls and
...
For a focused refinement, inspect the exact changed workflow at a representative
viewport and exercise the affected interaction.
