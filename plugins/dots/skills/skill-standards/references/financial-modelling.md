...
Read this when creating, updating, reviewing, or evaluating a skill that builds,
updates, audits, or explains a financial model. Use it to add the controls the
recurring job needs; it is not a workbook template and does not require every
model to use the same tabs, schedules, methods, or outputs.
...
## Failure behavior and evaluator cases
...
the expected formulas, controls, outputs, and failure behavior. Keep deterministic
