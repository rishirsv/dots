...
produce the assist answer locally. A CLI route does not need a saved Desktop
...
4. Pass the prompt text directly. Give the model file context only when the
   assist needs it and the CLI supports it; point it at the approved repo root,
...
```bash
claude -p "$(cat <<'PROMPT'
<standalone assist prompt with direct file references>
PROMPT
)" \
  --model opus \
  --effort high \
...
`answer.<provider>.md` when useful, then run the SKILL's *After The Assist*
