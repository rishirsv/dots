...
A deterministic structural task in skill-doctor's Verify tests (aggregated by
run.py; see ../references/rubric.md). These checks need no judge, so the LLM
scores only Discovery and Implementation while structure is verified
deterministically here. Pure stdlib — no third-party deps (PyYAML is used when
present, with a line-based fallback otherwise).

Usage:
    python3 validate_skill.py <path-to-SKILL.md | path-to-skill-dir> [--json]
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Validate an agent skill's SKILL.md structure.")
