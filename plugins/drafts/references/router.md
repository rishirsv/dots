...
boundary, supporting material, or revision target.
...
| `Rewrite` | User says rewrite, humanize, personalize, polish, tighten, shorten, expand, adapt, tell me what to fix and fix it, make this sound like me, make this less AI, or provides existing text to improve. | `compose` |
| `Style` | User asks to create, inspect, sync, import, export, or update a writing voice guide. | `writing-voice` |
...
- Prefer the Compose quality loop before returning writing. Do not route to
  another surface just to catch ordinary AI tells, weak endings, or style misses
  that Compose can fix before output.
...
missing, a style guide or samples are required for personalization, or the edit
...
Use these details to preserve safety around state, style, sources, and version
history. Do not print them in ordinary Draft or Rewrite responses. If the user
