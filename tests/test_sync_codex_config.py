...
class SyncConfigsIntegrationTests(unittest.TestCase):
    def test_codex_apply_and_status_copy_owned_files(self):
        with tempfile.TemporaryDirectory() as home_directory:
            ...
            self.assertIn("[permissions.dots.network]", live_config)
            self.assertIn("enabled = true", live_config)
            self.assertIn("allow_local_binding = true", live_config)
            self.assertIn("dangerously_allow_all_unix_sockets = true", live_config)
