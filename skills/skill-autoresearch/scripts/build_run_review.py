#!/usr/bin/env python3
"""Build the Autoresearch run-review site for the ``skill-autoresearch`` skill.
...
Usage:
    python skills/skill-autoresearch/scripts/build_run_review.py [--out <dir>]
"""
...
def main():
    ...
    ap.add_argument("--combined", action="store_true",
                    help="Also include autoresearch runs in the same site.")
    args = ap.parse_args()

    skills = ["skill-autoresearch"]
    if args.combined:
        skills.insert(0, "autoresearch")

    source_specs = {}
    if args.benchmark_run or args.observed_usage:
        source_specs["skill-autoresearch"] = {
