# Tessl React Dev Eval And Quality Reference

Captured from Chrome on the Tessl registry page:

- https://tessl.io/registry/skills/github/softaworks/agent-toolkit/react-dev/evals
- https://tessl.io/registry/skills/github/softaworks/agent-toolkit/react-dev/quality
...
Tessl quality is separate from task eval scenarios. It scores discovery, implementation, and validation.
...
Tessl's structure suggests two separate artifacts:
...
Executable `.meta-skill/evals/<id>/eval.md` files should then preserve the same task/criteria split, while run evidence can later compare `--no-skill` and skill-mounted runs per criterion.
