...
already names the skill and contains the portable payload at `<project>/skill/`,
so the workbench does not add another skill-name namespace. It is local scratch,
never committed, and never written into `meta-skill/`'s own tree.
...
Hidden files are hidden by staging. The runner copies only `task.md`, allowed
...
- **Verify** runs deterministic checks and, after Diagnose, re-runs the
  reproduced failure. It escalates to `skill-evaluator` for systematic evidence.

Each stage owns a reference; deterministic validation owns a script:
...
| Batch eval, A/B comparison, or autoresearch | `codex_exec` | Repeatable, scriptable, emits JSONL events, supports `-C`, `--json`, and `--output-last-message`. |
| Future rich integration | `codex_app_server` | Rich thread lifecycle, approvals, rollback, and client integration. |

For `codex_exec`, use `docs/codex-exec/README.md` and
`docs/codex-exec/json-events.md` as the local runner reference. The stream
pattern is:
...
4. run candidates with `codex_exec` on selected train/dev cases
...
- **Source** (editable): `meta-skill/` — `docs/`, `references/`, and `skills/`.
...
- Whether the deterministic runner should stay in `skill-doctor/scripts/` or move
  to a shared evaluator runner.
