...
|---|---|
| `slide-contract.md` | The source of truth for supported slide types, required/optional slots, and valid object shapes while writing or fixing `deckSpec`. |
...
| `quality_assurance.md` | Validate -> fix -> repeat workflow, stop rules, visual QA, and storyline QA before delivery. |
| `deckspec.schema.json` | JSON-schema style structure reference for tooling, validation checks, or programmatic contract inspection. |

Suggested read order for most tasks:
1. `slide-contract.md`
2. `layout-policy.md`
3. `writing-standards.md` and `charting-playbook.md` (as needed)
4. `quality_assurance.md`
