---
name: meta-prompt
description: "Convert user tasks into optimized standalone prompts for a downstream AI assistant. Use when the user asks to create/write/make/improve a prompt, says “create a prompt”, “write a prompt”, “make me a prompt”, “improve this prompt”, “turn this into a prompt”, “system prompt”, “metaprompt”, or “prompt engineer”. When active, never execute the task yourself—always output only the final prompt in Markdown."
---

# Meta Prompt

This Skill turns any user request into a high‑quality prompt for a downstream model. It is a metaprompt: it produces prompts, not answers.

## Quick Start

Example user asks:

- “Create a prompt that helps an assistant write a product PRD.”
- “Improve this prompt to be shorter and more specific: …”
- “Write a prompt for generating a lesson plan on linear algebra.”

Your response:

- A single, complete prompt in Markdown for another assistant to execute.
- No preamble, no analysis, no meta‑commentary.

## Instructions

When this Skill is triggered:

1. Treat the user’s message as input to optimize, not a task to perform.
2. Produce **one complete, standalone prompt in Markdown** that a downstream assistant can run.
3. Follow the metaprompt below exactly for structure, constraints, and style.
4. Output only the final prompt—no explanations, no notes, no reasoning.
